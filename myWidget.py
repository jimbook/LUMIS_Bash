...
class subPlotWin_singal(Ui_Form,QWidget):
    ...
    # 设置数据和参数，初始化界面时用
    def setData(self,tier: int,channel: str,HighGain: bool):
        '''
            :param data: 数据
            :param tier: 层数
            :param channel: 通道
        ...
        #载入内存数据
        listOfData = np.array(dataInMemory).reshape((-1, 219))
    ...
    def dataUpdate(self):
        global dataInDisk,dataInMemory
        length = len(dataInMemory)
        # 如果数据只在内存中更新,只计算新增的数据
        if length >= self.index_memory:
            listOfData = np.array(dataInMemory[self.index_memory:]).reshape((-1, 219))
            d = pd.DataFrame(listOfData, columns=[_chnList, _typeList])
            self.index_memory = length
            self.energy_data_memory = self.energy_data_memory + self.selectData(d)[0]
        else:
            # 如果内存中的数据已经达到32MB而清空过，读取硬盘中最新的那个数据文件，同时读取内存中的数据
            # 读取硬盘中新文件的数据
            _dataFromFile = pd.read_csv(dataInDisk[-2],index_col=0,header=[0,1])
...
class subPlotWin_coincidence(Ui_Form,QWidget):
    ...
    # 设置数据和参数，初始化界面时用
    def setData(self, tier: int, channel: str, channel_coin: str,HighGain: bool):
        '''
            :param data: 数据
            :param tier: 层数
            :param channel: 通道
    ...
    def selectData(self, d: pd.DataFrame):
        _d = d[self.channel][self.HighGain]
        index = (d["SCAinfo"]["BoardID"] == self.tier) & \
                (d[self.channel][self.HighGain + "_hit"] == 1) & \
                (d[self.channel_coin][self.HighGain + "_hit"] == 1)
        _d = _d.values[index]  # 将目标板子和通道已触发的数据筛选出
        y, x = np.histogram(_d, bins=np.linspace(0, 2 ** 12, 2 ** 12))
        return y, x
